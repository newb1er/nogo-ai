...
class statistics {
 ...
 public:
   ...
   *
   * the format is
   * 1000   win = 53.5%|46.5%, op = 74.451 (37.493|36.958), ops = 125762
   * (132018|135377)
   *
   * where (block = 1000 by default)
   *  '1000': current index (n), i.e., this line is the statistic of game 1 ~
   * 1000 'win = 53.5%|46.5%': the win rate for black is 53.5%; for white
   * is 46.5% 'op = 74.451 (37.493|36.958)': the average move is 74.451 the
   * average move of black is 37.493 the average move of white is 36.958 'ops =
   * 125762 (132018|135377)': the average speed is 125762 the average speed of
   * black is 132018 the average speed of white is 135377
   */
  void show(size_t blk = 0) const {
    ...
    std::cout << "win = " << (BW * 100.0 / num) << "%"
              << "|" << (WW * 100.0 / num) << "%, ";
    std::cout << "op = " << (sop * 1.0 / num) << " (" << (Bop * 1.0 / num)
              << "|" << (Wop * 1.0 / num) << "), ";
    std::cout << "ops = " << (sop * 1000.0 / sdu) << " ("
              << (Bop * 1000.0 / Bdu) << "|" << (Wop * 1000.0 / Wdu) << ")";
    std::cout << std::endl;
  }

  void summary() const { show(data.size()); }
