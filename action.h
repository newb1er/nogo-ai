...
class action::place : public action {
 public:
  ...
  place(int i, unsigned who)
      : action(place::type | ((who & 0xff) << 16) | (i & 0xffff)) {}
  ...
  board::point position() const {
    return board::point(int16_t(event() & 0xffff));
  }
  board::piece_type color() const {
    return static_cast<board::piece_type>(event() >> 16);
  }

 public:
  board::reward apply(board& b) const { return b.place(position(), color()); }
  std::ostream& operator>>(std::ostream& out) const {
    return out << ';' << "?BW?"[color() & 0b11] << '['
               << char('a' + position().x)
...
class action::black : public action::place {
 ...
 protected:
  action& reinterpret(const action* a) const {
    return *new (const_cast<action*>(a)) black(*a);
  }
  static __attribute__((constructor)) void init() {
    entries()[type_flag('B')] = new black;
  }
};
...
class action::white : public action::place {
 ...
 protected:
  action& reinterpret(const action* a) const {
    return *new (const_cast<action*>(a)) white(*a);
  }
  static __attribute__((constructor)) void init() {
    entries()[type_flag('W')] = new white;
  }
};
